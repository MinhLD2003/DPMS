...
import VersionUpdateModal from '../../../components/common/popup-modals/VersionUpdateModal';
import { FormStatus } from '../../../enum/enum';
...
const FICTemplateUpdate = () => {
    const { t } = useTranslation();
    const navigate = useNavigate();
    const { id } = useParams(); // Get the FIC template ID from URL parameters
    ...
    const addIdsToStructure = useCallback((structure: FICStructure): ExtendedFICStructure => {
        const addIdsToElements = (elements: FormElement[]): ExtendedFormElement[] => {
            // Sort elements by orderIndex first before mapping
            return [...elements]
                .sort((a, b) => a.orderIndex - b.orderIndex)
                .map(elem => ({
                    ...elem,
                    id: generateUniqueId(),
                    // Normalize dataType to lowercase if it exists
                    dataType: elem.dataType ? elem.dataType.toLowerCase() : elem.dataType,
                    // Recursively sort and process children
                    children: elem.children ? addIdsToElements(elem.children) : []
                }));
        };
        return {
            ...structure,
    ...
    // Fetch FIC template details when component mounts
    useEffect(() => {
        const fetchFICTemplate = async () => {
            if (id) {
                try {
                    ...
                    if (response.data) {
                        const fetchedStructure = addIdsToStructure(response.data);
                        setStructure(fetchedStructure);
                        setFormName(fetchedStructure.name);

                        // Validate leaf nodes on load
                        validateLeafNodesHaveDataType(fetchedStructure);

                        message.success(t('ficTemplateConfig.templateLoaded'));
                    }
                } catch (error) {
                    console.error("Error fetching FIC template:", error);
                    message.error(t('ficTemplateConfig.templateLoadFailed'));
                } finally {
    ...
    const addElement = () => {
        ...
        // Validate after adding new element
        validateLeafNodesHaveDataType(newStructure);
    };
    ...
    const toggleOption = (path: string[], optionIndex: number) => {
        ...
        if (element) {
            ...
            const updatedInvalidNodes = new Set(invalidLeafNodes);
            updatedInvalidNodes.delete(element.id);
            setInvalidLeafNodes(updatedInvalidNodes);
        }
    };
    ...
    const validateLeafNodesHaveDataType = useCallback((structureToValidate: ExtendedFICStructure = structure): { isValid: boolean, invalidCount: number, invalidNodeIds: Set<string> } => {
        const invalidNodes = new Set<string>();

        const checkElements = (elements: ExtendedFormElement[]) => {
            elements.forEach(elem => {
                if (elem.children.length === 0) {
                    if (!elem.dataType) {//                // Stricter check to catch null, undefined, empty string, or any falsy value
        ...
        checkElements(structureToValidate.formElements);
    ...
    const handleSubmit = async () => {
        try {
            ...
            // If in edit mode, show the confirmation modal
            if (isEditMode && id) {
                setUpdateModalVisible(true);
            } else {
                // For new forms, proceed directly
                proceedWithSave();
            }
    ...
    // Add a new function to handle the actual save process
    const proceedWithSave = async () => {
        try {
            setLoading(true);
            const newTemplateData = prepareForSubmission(structure);

            if (isEditMode && id) {
                await AxiosClient.post(`/Form/update`, newTemplateData);
                message.success(t('ficTemplateConfig.formUpdated'));
            } else {
                await AxiosClient.post("/Form/save", newTemplateData);
                ...
                <React.Fragment key={elem.id}>
                    ...
                    `}>
                        ...
                        <td className="px-2 py-3 text-center">
                            {isLeaf && (
                                <Checkbox
                                    checked={elem.dataType?.toLowerCase() === 'boolean'}
                        ...
                        <td className="px-2 py-3 text-center">
                            {isLeaf && (
                                <Checkbox
                                    checked={elem.dataType?.toLowerCase() === 'text'}
        ...
        return (
            <Layout style={{ height: '100vh', backgroundColor: '#f5f7fa' }}>
                <div className="flex items-center justify-center h-full">
                    <Spin tip={t('ficTemplateConfig.loading')} size="large" />
                </div>
            </Layout>
        );
    }
    ...
    return (
        <Spin spinning={loading} tip={t('processing')}>
            <Layout style={{ minHeight: '100vh', backgroundColor: '#f5f7fa', borderRadius:'12px'}}>
                <PageHeader
                    title={isEditMode ? t('ficTemplateConfig.editPageTitle') : t('ficTemplateConfig.pageTitle')}
                />
                <div className='px-4 py-8'>
                    <Card className="mb-6 shadow-sm">
                        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4">
                            <h1 className="text-2xl font-bold text-gray-800 mb-4 sm:mb-0">
                                {isEditMode ? t('ficTemplateConfig.editPageTitle') : t('ficTemplateConfig.pageTitle')}
                            </h1>
                        ...
                        <div className="bg-yellow-50 p-3 rounded-lg border border-blue-200 mb-4">
                            <h2 className="text-xl font-semibold text-blue-800">
                                {t('ficTemplateConfig.currentForm')}: {structure.name}
                            </h2>
                    ...
                    <div className="flex justify-center mt-6">
                        <Button
                            ...
                            className="px-8"
                        >
                            {isEditMode ? t('ficTemplateConfig.updateTemplate') : t('ficTemplateConfig.saveTemplate')}
                        </Button>
                    </div>
                </div>
                <VersionUpdateModal
...
export default FICTemplateUpdate;