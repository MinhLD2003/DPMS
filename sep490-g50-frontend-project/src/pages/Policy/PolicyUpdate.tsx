...
const UpdatePolicy: React.FC = () => {
    ...
    useEffect(() => {
        const fetchPolicyData = async () => {
            try {
                setLoading(true);
                const response = await AxiosClient.get(`/PrivacyPolicy/${id}`);
                console.log("API Response:", response.data);
                
                // Case 1: API returns { success: true, data: {...} }
                if (response.data && response.data.policyCode) {
                    ...
                    setEditorData(policyData.content || '');
                }
                else {
                    console.error("Unexpected API response structure:", response.data);
                    message.warning("Policy data format is unexpected. Some fields may not display correctly.");
                }
            } catch (error: any) {
                console.error('Error fetching policy:', error);
                message.error('Failed to load policy data: ' + (error.message || 'Unknown error'));
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleUpdate = async () => {
        try {
            ...
            try {
                ...
                const response = await AxiosClient.put(`/PrivacyPolicy/${id}`, values);
    ...
    if (loading) {
        return (
            <div className="update-policy-container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '50vh' }}>
                <Spin size="large" tip="Loading policy data..." />
            </div>
        );
    }
                    ...
                    <Row justify="space-between" align="middle">
                        <Col>
                            <Space align="center">
                                ...
                                <Title level={2} style={{ margin: 0 }}>Update Policy</Title>
                            </Space>
                        </Col>
                    </Row>
                </div>
